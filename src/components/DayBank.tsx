    ...
    <div className="save-load-section">
      ...
      <form onSubmit={handleSave} style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
        ...
        <input
          ...
          style={{ width: 140 }}
        />
        <button type="submit" className="btn-primary">Save today&apos;s state</button>
      </form>
      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
        {sorted.map((d) => (
          <li key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <button type="button" className="btn-ghost" onClick={() => onLoadDay(d)}>Load</button>
            ...
            {onRemoveDay && (
              <button type="button" className="btn-icon btn-danger" onClick={() => onRemoveDay(d.id)} aria-label="Remove">×</button>
            )}
          </li>
        ))}
      </ul>
